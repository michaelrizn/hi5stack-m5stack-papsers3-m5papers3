...
- `cd webassembly && make all`: build all supported games for the browser emulator.
- `cd webassembly && make minesweeper`: build one wasm game target.
- `cd webassembly && ./start_emulator.sh`: start the local emulator workflow.
...
There is no dedicated unit test framework in this repository. Treat `pio run` as the required compile check before submitting changes. For UI or game changes, verify behavior on device when possible. For games with emulator support, run the relevant `webassembly` Make target and test in the browser. Keep game entry points compatible with the emulator pattern: `initGame()`, `drawGameScreen()`, `handleTouch(...)`, and `resetGame()`.

## Commit & Pull Request Guidelines

Recent history uses short release-style commits such as `0.40` and occasional conventional messages like `feat(swipe_test): ...` or `refactor: ...`. Prefer clear, scoped commits: `feat(reader): add page jump`, `fix(sd): handle missing card`, or `refactor(ui): simplify footer redraw`. Pull requests should describe the changed firmware behavior, mention hardware tested, include build results, and attach screenshots or emulator notes for visible UI changes.
